\begin{abstract}
Domain-Specific Languages (DSLs) manifest themselves in remarkably diverse \emph{shapes} ranging from internal DSLs embedded as a mere fluent API within a programming language to external DSLs with dedicated syntaxes and tool support.
% engineered using one of many language workbenches.
%
Although all shapes have pros and cons, combining them for a single language is problematic:~language designers usually commit to a particular shape early in the design process and this choice can hardly be reconsidered later on.
%
...
%
In this \emph{new idea} paper, we envision a language engineering approach enabling (i) language users to manipulate their abstractions in the most appropriate shape according to the task at hand and (ii) language designers to combine the strengths of different technologies in the creation of a single DSL. 
...
%
We report on early experiments and lessons learned building \prism, an incremental synchronization mechanism for shape-diverse DSLs, and its applicability in the engineering of a simple shape-diverse DSL implemented in Rascal, EMF, and Java.
...
%
We intend to raise the awareness of the community on these notions, identify some of the challenges we encountered along the way and, hopefully, encourage future investigations.
\end{abstract}
