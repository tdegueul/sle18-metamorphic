\begin{abstract}
Domain-Specific Languages (DSLs) manifest themselves in remarkably diverse \emph{shapes} ranging from internal DSL embedded as a mere fluent API within a programming language to external DSLs with dedicated syntax and tool support.
% engineered using one of many language workbenches.
%
Although all shapes have pros and cons, combining them for a single language is problematic:~language designers usually commit to a particular technology and a particular shape early in the design process and this choice can hardly be reconsidered later on.
%
...
%
In this \emph{new idea} paper, we envision the support of multiple shapes for a single DSL to better fit the various tasks to be performed with it. \td{Meh}
...
%
We report on early experiments and lessons learned building \prism, an incremental synchronization mechanism for multi-shapes \td{?} DSLs.
