...
by Logic Synthesis using SAT solvers~\cite{DBLP:conf/etaps/SemerathDHV16}, which propagate changes from models to views using traceability links. This is in contrast with our approach where the underlying model is not materialized and incarnations exchange patch each other.
...
Intuitively, our approach supposes that all the information that does not directly relate to the constructs of a language is ``extra'' and should not be part of the patch itself.
Sharing extra-information from one shape to the other might be desirable, for instance to share layout information between textual editors.
